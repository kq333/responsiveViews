    ...
    <main>
      ...
      <article>
        <header class="sectionNav">
          <h1 class="sectionNav__header">Collection</h1>
      ...
      <article>
        <section class="itemCollection">
          <div v-for="n in 4" :key="n">
            <div>
            <ImageComponent :getData="getData[0]" :class="$style.imageSizeSection" />  </div>
            <p>{{getData[0].title}}</p>
          </div>
        </section>
      </article>
...
.itemCollection {
  ...
  @media(max-width:576px){
     ...
     grid-gap: 8%;
      display: grid;
    //  grid-template-columns: repeat2,auto;
   grid-template-columns: repeat(2, auto-fit), minmax(250px, 1fr) );
