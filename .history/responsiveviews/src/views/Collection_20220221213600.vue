<template>
  <div class="Collection">
    ...
    <main>
     <!--   <router-link :to="{name:'CollectionDetails', params:{id: 1}}">sasa</router-link> -->
      <section>
        <article class="itemCollection">

          <div

          class="itemCollection__item"
          v-for="n in 6"
          :key="n">

            <ImageComponent :getData="getData[0]" :class="$style.imageSize" />
            <router-link :to="{name:'CollectionDetails', params:{id: getData[0].id}}">{{ getData[0].title }}</router-link>
          </div>
        </article>
      </section>
    </main>
